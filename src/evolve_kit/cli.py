...
def cmd_dashboard(engine: EvolutionEngine, args: argparse.Namespace) -> None:
    ...
    mem = engine.memory.count()
    avg = engine.scores.average_quality()
    lvl = max(1, int(report.total_score // 10))

    dim_icons = {
        "insight_loop": "👁️", "memory_lifecycle": "🧠",
        "self_challenge": "⚔️", "reflection": "🪞",
        "tool_lifecycle": "🔧", "architecture_search": "🏗️",
        "test_time_learning": "📚", "population_diversity": "👥",
        "safety_guardrails": "🛡️", "co_evolving_eval": "📈",
    }

    dim_names = {
        "insight_loop": "洞察力", "memory_lifecycle": "记忆管理",
        "self_challenge": "自我挑战", "reflection": "反思能力",
        "tool_lifecycle": "工具管理", "architecture_search": "架构探索",
        "test_time_learning": "学习速度", "population_diversity": "群体多样性",
        "safety_guardrails": "安全守卫", "co_evolving_eval": "进化趋势",
    }
    ...
    for a in achievements:
        ach_html += f'<div class="achievement">{a["icon"]} {a["name"]}</div>'
...
<div class="card">
  <div class="header">
    <h1>🧙 AGENT-{lvl:02d}</h1>
    <div class="level">Lv.{lvl} ⭐{'⭐' * int(avg)} | {report.level}</div>
  </div>
  <div class="xp-bar"><div class="xp-fill" style="width:{report.total_score}%"></div></div>
  <div class="xp-label">EXP {report.total_score:.0f}/100</div>
  <div class="stats">
    <div class="section-title">⚔️ ATTRIBUTES</div>
    {"".join(dim_rows)}
  </div>
  <div class="section-title">🏆 ACHIEVEMENTS ({len(achievements)}/20)</div>
